resource "yandex_compute_instance" "ubuntu-node" {
  count                     = local.workspace["vm_count"]
  name                      = "ubu-node-${terraform.workspace}-0${count.index + 1}"
  zone                      = var.yandex_zone
  hostname                  = "ubu-node-${terraform.workspace}-0${count.index + 1}"
  allow_stopping_for_update = true

  description = "${terraform.workspace} node for 7.3 HW"
  ...
  network_interface {
    ...
    nat        = true
    ip_address = "${local.workspace["subnet_addr"]}.${count.index + 11}"
  }
